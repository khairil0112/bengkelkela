...
@section('content')
    ...
    <form action="{{ route('pembelian.update', $pembelian->id_pembelian) }}" method="POST">
        ...
        <div class="row mt-3">
            <div class="col-md-6 mb-3">
                ...
                <input type="text" class="form-control" name="kode_pembelian" value="{{ $pembelian->kode_pembelian }}"
                    required>
            </div>
            ...
            <div class="col-md-6 mb-3">
                ...
                <select name="idpemasok" class="form-select" required>
                    ...
                    @foreach ($pemasok as $p)
                        <option value="{{ $p->idpemasok }}" {{ $pembelian->idpemasok == $p->idpemasok ? 'selected' : '' }}>
                            {{ $p->nama }}
                        </option>
                    @endforeach
                </select>
                ...
                @if ($pembelian->details->count() > 0)
                    @foreach ($pembelian->details as $detail)
                        <tr>
                                ...
                                <select name="part_id[]" class="form-select partSelect select2" required>
                                    ...
                                    @foreach ($part as $p)
                                        <option value="{{ $p->id }}" data-harga="{{ $p->hbterakhir }}"
                                            {{ $detail->partjasa_id == $p->id ? 'selected' : '' }}>
                                            {{ $p->nama }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <td>
                                <input type="number" name="qty[]" class="form-control qty" value="{{ $detail->qty }}"
                                    min="1">
                            </td>
                            <td>
                            ...
                            <td>
                                <input type="number" class="form-control subtotal" value="{{ $detail->subtotal }}"
                                    readonly>
                            </td>
                            <td>
        ...
        <h4 class="mt-3 text-end">
            Total Pembelian: Rp <b id="grandTotal">{{ number_format($pembelian->grand_total, 0, ',', '.') }}</b>
        </h4>

        <input type="hidden" name="grand_total" id="grand_total" value="{{ $pembelian->grand_total }}">
    ...
    <script>
        $(function() {
            ...
            // Trigger change for pre-selected partSelect to set harga
